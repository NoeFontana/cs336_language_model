...
def validate_ddp_net_equivalence(net):
    ...
    for t in net_module_states:
        tensor_list = [torch.zeros_like(t) for _ in range(dist.get_world_size())]  # ty: ignore[possibly-missing-attribute]
        dist.all_gather(tensor_list, t)  # ty: ignore[possibly-missing-attribute]
...
def _setup_process_group(rank, world_size, backend):
    ...
    dist.init_process_group(backend, rank=rank, world_size=world_size)  # ty: ignore[possibly-missing-attribute]
    return device
