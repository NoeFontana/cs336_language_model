...
def _test_flash_forward_pass(impl, device="cpu", is_causal=False):
    ...
    assert o.grad_fn.saved_tensors is not None, (
        "No saved tensors found in the output tensor."
        "Make sure your autograd forward is saving them using ctx.save_for_backward."
    )
    maybe_ls = [t for t in o.grad_fn.saved_tensors if t.shape == (q.shape[0], q.shape[1])]

    assert len(maybe_ls) == 1, (
        f"Expected one tensor of shape {q.shape[0], q.shape[1]} in saved tensors, but found {len(maybe_ls)}."
        "The tests require you to save exactly one tensor of this shape, "
        "corresponding to the log-sum-exp of the attention scores."
    )
    tensor = maybe_ls[0]
...
def test_flash_forward_pass_triton(is_causal):
    _test_flash_forward_pass(get_flashattention_autograd_function_triton().apply, device="cuda", is_causal=is_causal)  # ty: ignore[unresolved-attribute]
...
def test_flash_backward_pytorch():
    ...
    get_flashattention_autograd_function_pytorch().apply(q, k, v, False).backward(do)  # ty: ignore[unresolved-attribute]
...
def test_flash_backward_triton(is_causal):
    ...
    get_flashattention_autograd_function_triton().apply(q, k, v, is_causal).backward(do)  # ty: ignore[unresolved-attribute]
